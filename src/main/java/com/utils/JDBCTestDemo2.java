...
//                e.printStackTrace();
//            }
            Date date = new MyDateTimeUtils().getNewDateByMinute(new Date(),-1);
            createTime = new MyDateTimeUtils().DateTimeToStr(date, "yyyy-MM-dd HH:mm:ss").replace("\\s*", "");

        }
        String tzSql = "select THEME_ID, THEME_NAME, POST_BAR, REPLY_ID, POST_BAR_CLASS, BELONG_FLOOR, PAGE_NUM, REPLY_TIME, REPLY_CONTENT, "
        ...
        tzSql = "select THEME_ID, THEME_NAME, POST_BAR, REPLY_ID, POST_BAR_CLASS, BELONG_FLOOR, PAGE_NUM, REPLY_TIME, REPLY_CONTENT, "
                + "PAGE_URL from A_POST_BAR_JX3_2 where 1=1 order by CREATETIME desc limit 10000";
//		String tzSql = "select THEME_ID, THEME_NAME, POST_BAR, REPLY_ID, POST_BAR_CLASS, BELONG_FLOOR, PAGE_NUM, REPLY_TIME, REPLY_CONTENT, "
//				+ "PAGE_URL from A_POST_BAR_JX3_2 where theme_id='e0a73685614544b811f2aaece744994e' and BELONG_FLOOR= 5107";
...
//					}
                    ...
                    if (wgCount > 0 && isContinue) {
                        ...
                        String sqlWg = "insert into C_POST_BAR_13(RECORD_ID,THEME_ID,THEME_NAME,POST_BAR,POST_ID,POST_BAR_CLASS,BELONG_FLOOR,PAGE_NUM,REPLY_TIME,"
                                + "POST_CONTENT,PAGE_URL,VIEW_NAME,WAIGUAN_NAME,MAIN_ID,TRADE_TYPE,BELONG_QF,url_valid,source_type,FAVOR_ID,createtime,updatetime) "
                                + "values(UUID(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,nextval('SEQUENCE_3'),sysdate(),sysdate())";
                        List<Object[]> batchArgsList = Lists.newArrayList();
                            ...
                            if (waiguan.size() > 1) {
                                ...
                                for (String splitData : splitDatas) {
                                    ...
                                    if(waiguan.size()>0){
                                        ...
                                        args[13] = trade_type;
                                        args[14] = qufu.size()>0?qufu.iterator().next() + "":"";
                                        batchArgsList.add(args);
                                    }
                    ...
                    if (djCount > 0 && isContinue) {
                        ...
                        String sqlDj = "insert into C_POST_BAR_15(RECORD_ID,THEME_ID,THEME_NAME,POST_BAR,POST_ID,POST_BAR_CLASS,BELONG_FLOOR,"
                                + "PAGE_NUM,REPLY_TIME,POST_CONTENT,PAGE_URL,PROP_NAME,PRICE_NUM,TRADE_TYPE,MAIN_ID,BELONG_QF,source_type,url_valid,FAVOR_ID,createtime,updatetime) "
                                + "values (UUID(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,nextval('SEQUENCE_3'),sysdate(),sysdate())";
                        List<Object[]> batchArgsList = Lists.newArrayList();
                        ...
                        try {
                            if (daoju.size() > 1) {
                                ...
                                for (String splitData : splitDatas) {
                                    ...
                                    args[8] = splitData;
                                    args[9] = PAGE_URL;
                                    args[10] = Arrays.toString(daoju.toArray());
                    ...
                    if (dlCount > 0 && isContinue) {
                        String sqlDL = "insert into C_POST_BAR_17(RECORD_ID,THEME_ID,THEME_NAME,POST_BAR,POST_ID,POST_BAR_CLASS,BELONG_FLOOR,PAGE_NUM,REPLY_TIME,"
                                + "POST_CONTENT,PAGE_URL,Main_id,BELONG_QF,source_type,FAVOR_ID,createtime,updatetime) values(UUID(),?,?,?,?,?,?,?,?,?,?,?,?,1,nextval('SEQUENCE_3'),sysdate(),sysdate())";
                        List<Object[]> batchArgsList = Lists.newArrayList();
                        ...
                        try {
                            if (dailian.size() > 1) {
                                ...
                                for (String splitData : splitDatas) {
                                    ...
                                    args[8] = splitData;
                                    args[9] = PAGE_URL;
                                    args[10] = Md5Util2.getMd5(REPLY_ID+splitData);
                                    args[11] = qufu.size()>0?qufu.iterator().next() + "":"";
                                ...
                                args[8] = data.length() < 1000 ? data : data.substring(0, 1000);
                                args[9] = PAGE_URL;
                                args[10] = Md5Util2.getMd5(REPLY_ID+data);
                                args[11] = qufu.size()>0?qufu.iterator().next() + "":"";
